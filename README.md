...
```
backend/app/  – FastAPI REST API (wraps manager pipeline)
frontend/     – React + Vite + TypeScript web UI
models/
  ocr/        – Model A: OCR-based lab-value extraction (PaddleOCR)
  diagnosis/  – Model B: diagnosis inference (rule-based + optional RAG)
...
1. Install API deps: `pip install -r backend/requirements.txt` (after core `pip install -r requirements.txt`).
2. Start API from repo root: `uvicorn app.main:app --reload --app-dir backend --port 8000`
3. UI: `cd frontend && npm install && npm run dev` → open `http://127.0.0.1:5173`

See `backend/README.md` and `frontend/README.md` for details. Swagger docs: `http://127.0.0.1:8000/api/docs`.
