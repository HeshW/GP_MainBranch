...
_REAL_CHAT_SYMPTOM_ALIASES: dict[str, tuple[str, ...]] = {
    "fatigue": ("wiped out", "no energy", "drained"),
    "weakness": ("feel weak", "legs feel weak"),
    "nausea": ("sick to my stomach", "feel sick"),
    "vomiting": ("throwing up", "threw up", "can't keep food down", "cannot keep food down"),
    "fever": ("temperature", "burning up", "chills", "hot and shivery"),
    "cough": ("can't stop coughing", "cannot stop coughing"),
    "shortness of breath": (
        "breathless",
        "hard to breathe",
        "trouble breathing",
    ...
    "wheezing": ("whistling in my chest",),
    "chest pain": ("pain in my chest", "sharp pain in my chest", "chest pressure"),
    "chest tightness": ("tightness in my chest", "heavy chest", "chest feels heavy"),
    "palpitations": (
        "heart racing",
        "heart is racing",
        "heart races",
    ...
    "sore throat": ("scratchy throat", "throat is scratchy", "pain when swallowing"),
    "hoarseness": ("lost my voice", "voice is gone"),
    "abdominal pain": ("belly pain", "tummy pain", "stomach ache", "stomach cramps"),
    "loss of appetite": ("not hungry", "can't eat much", "cannot eat much"),
    "weight loss": ("lost weight", "losing weight"),
    "nasal congestion": ("blocked nose", "runny nose", "stuffy"),
    "facial pressure": ("pressure around my face", "pressure in my face"),
    "ear pain": ("earache",),
    "tingling": ("numbness", "numb"),
    "gait difficulty": ("unsteady walking",),
    "ptosis": ("eyelids droop", "eyelid droops"),
    "reflux": ("burning in my chest", "acid coming up", "sour burps"),
    "thirst": ("always thirsty", "drinking all the time"),
    "polyuria": ("peeing a lot", "urinating a lot", "going to the bathroom a lot"),
    "blurred vision": ("vision is blurry",),
    "leg swelling": ("swollen ankles", "ankle swelling", "ankles are swollen", "legs are swollen", "calf swelling", "calf is swollen"),
}

SYMPTOM_PATTERNS = [
    (
        canonical,
        tuple(dict.fromkeys((*aliases, *_REAL_CHAT_SYMPTOM_ALIASES.get(canonical, ())))),
    )
...
def _extract_context(raw_text: str) -> Dict[str, List[str]]:
    patterns = {
        "duration": [
            ...
            r"\b(?:last|past)\s+\w+\s+(?:day|days|week|weeks|month|months|year|years)\b",
        ...
        "triggers": [
            ...
            r"\bworse when breathing\b",
            r"\bworse when i breathe\b",
            r"\bdeep breath\b",
            r"بعد الأكل",
            r"مع المجهود",
            r"مع التنفس",
