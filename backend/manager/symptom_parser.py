...
def _get_symptom_alias_index() -> Optional[Dict[str, str]]:
    """Return the cached fuzzy alias index for SYMPTOM_PATTERNS, or None if
...
def _extract_symptoms(raw_text: str) -> List[Dict[str, Any]]:
    ...
    # --- Fuzzy fallback: catch misspellings and OCR errors not handled above ---
    ...
    if alias_index is not None:
        try:
            ...
            for token in extract_candidate_tokens(text):
                ...
                if result is None:
                    continue
                canonical_match, _matched, _score = result
            ...
            pass  # rapidfuzz not available; skip fuzzy fallback
