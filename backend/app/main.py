...
def _has_faiss_assets(path_value: str | None) -> bool:
    if not path_value:
        return False
    path = Path(path_value)
    return (
        path.is_dir()
        and (path / "medical_cases.index").is_file()
        and (
            (path / "metadata_mapping.json").is_file()
            or (path / "metadata_mapping.pkl").is_file()
        )
    )
...
def _resolve_optional_ai_flags(settings) -> tuple[bool, bool, list[str]]:
    ...
    if use_rag and not _has_faiss_assets(settings.faiss_index_dir):
        warnings.append(
            "USE_RAG is enabled but FAISS_INDEX_DIR is missing required FAISS assets; disabling RAG for this startup."
        )
        use_rag = False
